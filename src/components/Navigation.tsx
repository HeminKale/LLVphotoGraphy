...
export default function Navigation() {
  ...
  const linkClass = (path: string) =>
    `font-sans text-sm tracking-wide uppercase transition-all hover:opacity-70 text-[#F1EDFB] border-[#8F1ED2] text-shadow-sm ${
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        isDarkHeader
          ? 'bg-[#100B21]/95 backdrop-blur-sm shadow-md py-3'
          : 'bg-transparent py-5'
      }`}
    >
      <div className="container mx-auto px-6">
        ...
        <div className="md:hidden relative flex items-center justify-center">
          ...
          <button
            ...
            className="absolute right-0 text-[#F1EDFB] drop-shadow-md transition-colors"
          >
            {isMobileMenuOpen ? (
              <X className="w-6 h-6" />
            ) : (
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-[#100B21] shadow-lg mt-4 animate-slide-down">
